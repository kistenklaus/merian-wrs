...
# Load the data
df = pd.read_csv("block_scan_benchmark_flushL2.csv")
...
df_min_latency = df.groupby(["N", group], as_index=False)[property].max()
...
for method in df_min_latency[group].unique():
    subset = df_min_latency[df_min_latency[group] == method]
