...
bench_unserious = pd.read_csv(f'./psa_inline_splitpack_benchmark_splitSizes.csv')
...
def merge_duplicates2(df):
    ...
    methods = df.groupby(["flushL2", "method", "splitSize", "threadsPerPack", "workgroupSize", "group", "N"]).apply(
        lambda g: pd.Series({
    ...
    groups = methods.groupby(["flushL2", "group", "splitSize", "threadsPerPack", "workgroupSize", "N"]).apply(
        lambda g: pd.Series({
...
def merge_duplicates(df):
    methods = df.groupby(["flushL2", "method", "splitSize", "workgroupSize", "group", "N"]).agg(
    ...
    groups = methods.groupby(["flushL2", "group", "workgroupSize", "splitSize", "N"]).agg(
            latency=("latency", "mean"),
            count=("std_derivation", "count"),  # Get count of elements per group
            var_sum=("std_derivation", lambda x: (x**2).sum())  # Sum of squared std deviations
            )
...
plt.tight_layout()
plt.savefig(f'psa_inline_splitpack_throughput_splitSizes.pdf', format="pdf")
