...
if optimizeFor=="splitPackLatency":
    bench["memory_throughput"] = (
            bench["N"] * (memory_per_item - memory_per_item_partition_prefix) + 
            (bench["K"] * memory_per_split * 2)
            ) / (bench[optimizeFor] * 1e-3) * 1e-9
else:
    bench["memory_throughput"] = (
            bench["N"] * (memory_per_item - memory_per_item_partition_prefix) + 
            (bench["K"] * memory_per_split )
            ) / (bench[optimizeFor] * 1e-3) * 1e-9
...
def plotMe(df, label, color):
    ...
    if onlyCached:
        plt.plot(fully_cached[axis], fully_cached[property], "-", label=label, color=color)
    else:
        plt.plot(fully_cached[axis], fully_cached[property], ":", color=color)
        plt.plot(partialy_cached[axis], partialy_cached[property], "-", label=label, color=color)
...
if property == "threads":
    plt.text(x=1.2e5, y=maxOccupantThreads*1.1, s="Max. occupant invocations")
...
plt.tight_layout()
plt.savefig(f'psa_inline_splitpack_optimal_splitSize.pdf', format="pdf")
