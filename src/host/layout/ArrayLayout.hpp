...
template <typename T, glsl::StorageQualifier Storage>
    requires((glsl::primitive_like<T> || traits::IsPrimitiveLayout<T>) != // xor
             layout::traits::IsSizedStructLayout<T>)
class ArrayLayout {
  public:
    ...
    static constexpr std::size_t size(std::size_t elementCount)
        requires layout::traits::IsSizedStructLayout<T>
    {
    ...
    template <typed_allocator<T> Allocator = std::allocator<T>>
    ...
    template <layout::traits::IsStorageCompatibleStruct<T> S>
