...
template <glsl::StorageQualifier Storage, typename... Attributes> class StructLayout {
  public:
    ...
    static constexpr bool sized = !layout::traits::IsLastAttributeArray<Attributes...>;
    ...
    template <layout::traits::IsStorageCompatibleStruct<StructLayout> S>
    void setMapped(void* mapped, const S& s) const
        requires layout::traits::IsSizedStructLayout<StructLayout>
    {
    ...
    static constexpr vk::DeviceSize alignment() {
        ...
        [&]<std::size_t... Is>(std::index_sequence<Is...>) {
            (([&]() {
                 ...
                 if constexpr (glsl::primitive_like<AttributeType>) {
                 ...
                 } else if constexpr (layout::traits::IsStructLayout<AttributeType>) {
    ...
    static constexpr vk::DeviceSize size(std::size_t arraySize)
        requires(layout::traits::IsUnsizedStructLayout<StructLayout>)
    {
        vk::DeviceSize total_size = 0;
        ...
        [&]<std::size_t... Is>(std::index_sequence<Is...>) {
            (([&]() {
                 ...
                 } else if constexpr (layout::traits::IsArrayLayout<AttributeType>) {
                     using Layout = AttributeType;
                     total_size =
                         alignUp(total_size, Layout::alignment()) + Layout::size(arraySize);
                 } else if constexpr (layout::traits::IsSizedStructLayout<AttributeType>) {
                     using Layout = AttributeType;
                     total_size = alignUp(total_size, Layout::alignment()) + Layout::size();
                 }
                 if constexpr (layout::traits::IsUnsizedStructLayout<AttributeType>) {
                     using Layout = AttributeType;
                     total_size =
                         alignUp(total_size, Layout::alignment()) + Layout::size(arraySize);
                 }
        ...
        [&]<std::size_t... Is>(std::index_sequence<Is...>) {
            ...
            (([&]() {
                 ...
                 if constexpr (glsl::primitive_like<AttributeType>) {
                     ...
                     offsets[Is] = alignUp(running_offset, AttributeLayout::alignment());
                     running_offset = offsets[Is] + AttributeLayout::size();
                 } else if constexpr (layout::traits::IsUnsizedStructLayout<AttributeType>) {
                     using AttributeLayout = AttributeType;
                     offsets[Is] = alignUp(running_offset, AttributeLayout::alignment());
                 }
             }()),
