...
template <std::totally_ordered T, generic_allocator Allocator> struct IsPartitionError {
...
template <std::totally_ordered T, generic_allocator Allocator = std::allocator<void>>
IsPartitionError<
    T,
...
assert_is_partition(std::span<const T> heavy,
    ...
    return test::assert_is_partition<T, std::pmr::polymorphic_allocator<void>>(
