...
std::pmr::vector<T> prefix_sum(const Range& weights,
    ...
    // Hope for RTO
    return reference::prefix_sum<T, std::pmr::polymorphic_allocator<T>>(weights, alloc);
}
...
std::pmr::vector<T> imperfect_prefix_sum(const Range& elements,
                                         T std_deviation,
                                         const std::pmr::polymorphic_allocator<T>& alloc = {}) {
    return reference::imperfect_prefix_sum<T, std::pmr::polymorphic_allocator<T>, Range>(
