...
template <arithmetic T,
          ...
          std::integral I,
          generic_allocator Allocator = std::allocator<AliasTableEntry<P, I>>>
AliasTable<P,
           I,
...
sweeping_alias_table(std::span<T> weights, const T totalWeight, const Allocator& alloc = {}) {
    using EntryAllocator =
        std::allocator_traits<Allocator>::template rebind_alloc<AliasTableEntry<P, I>>;
...
template <arithmetic T, std::floating_point P, std::integral I>
host::pmr::AliasTable<P, I> sweeping_alias_table(
    std::span<T> weights, const T totalWeight, const std::pmr::polymorphic_allocator<void>& alloc) {
    // Hope for RTO
    return reference::sweeping_alias_table<T, P, I, std::pmr::polymorphic_allocator<void>>(
