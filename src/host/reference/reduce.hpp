...
template <typename T> T neumaier_reduction(std::span<const T> elements) {
    T sum = 0;
    T c = 0; // Error term
...
template <typename T> T pairwise_kahan_reduction(const std::span<T> elements) {
...
template <typename T> T kahan_reduction(std::span<const T> elements) {
...
template <arithmetic T, typed_allocator<T> Allocator = std::allocator<T>>
    requires(std::same_as<typename Allocator::value_type, T>)
T reduce(std::span<const T> elements, [[maybe_unused]] const Allocator& alloc = {}) {
    return kahan_reduction<T>(elements);
}
...
inline T tree_reduction(const std::span<T> elements,
    ...
    return reference::tree_reduction(elements, alloc);
}
...
inline T block_reduction(const std::span<T> elements,
                         ...
                         const std::pmr::polymorphic_allocator<T>& alloc = {}) {
    return reference::block_reduction<T>(elements, blockSize, alloc);
}
