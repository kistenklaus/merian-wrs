...
template <std::floating_point E,
          std::integral I,
          host::typed_allocator<I> Allocator = std::allocator<I>>
E rmse(std::span<const E> weights,
    ...
    if (!totalWeight.has_value()) {
        totalWeight = host::reference::reduce<E>(weights);
    }
    ...
    auto histogram = host::histogram(samples, weights.size(), alloc);
...
template <std::floating_point E,
          ...
          host::generic_allocator Allocator = std::allocator<E>>
...
struct RMSECurveSectionedBuilder {
    ...
    explicit RMSECurveSectionedBuilder(const std::span<const W> weights, const Scale& scale)
        : m_probabilities(host::reference::weights_to_probabilities<W, E>(weights)),
...
struct RMSECurveAcceleratedBuilder {
    ...
    explicit RMSECurveAcceleratedBuilder(const merian::ContextHandle& context,
        ...
        m_histogram = alloc->createBuffer(device::AtomicHistogramBuffers::HistogramLayout::size(N),
                                          vk::BufferUsageFlagBits::eStorageBuffer |
                                              vk::BufferUsageFlagBits::eTransferSrc,
                                          merian::MemoryMappingType::NONE);

        m_histogramStage = alloc->createBuffer(
            device::AtomicHistogramBuffers::HistogramLayout::size(N),
    ...
    void appendHistogram(const merian::CommandBufferHandle cmd,
        ...
        m_s += count;
        device::AtomicHistogramBuffers buffers;
    ...
    void downloadMSE_toStage(const merian::CommandBufferHandle& cmd) {
        ...
        {
            device::AtomicHistogram::Buffers::HistogramView stageView{m_histogramStage, m_n};
            device::AtomicHistogram::Buffers::HistogramView localView{m_histogram, m_n};
            localView.expectComputeWrite();
            localView.copyTo(cmd, stageView);
            stageView.expectHostRead(cmd);
        }
    ...
    device::Histogram m_histogramKernel;
