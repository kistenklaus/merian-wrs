...
namespace device::test::block_wise_partition {
...
struct Results {
    std::pmr::vector<host::glsl::uint> indices;
    std::pmr::vector<host::glsl::uint> blockCount;
...
static Results downloadFromStage(Buffers& stage,
    ...
    auto indices =
        stagePrefixView.download<host::glsl::uint, host::pmr_alloc<host::glsl::uint>>(resource);

    Buffers::BlockIndicesView stageReductionView{stage.blockIndices, partitionCount};
    auto blockCount =
        stageReductionView.download<host::glsl::uint, host::pmr_alloc<host::glsl::uint>>(resource);
