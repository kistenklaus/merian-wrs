...
bool runTestCase(const TestContext& context,
    ...
    wrs::DecoupledMean kernel(context.context, context.shaderCompiler, testCase.config);
    ...
    for (size_t i = 0; i < testCase.iterations; ++i) {
        ...
        // Upload elements
        {
            ...
            uploadTestCase(cmd, elements, testCase.config.workgroupSize, testCase.config.rows, buffers, stage);
        }
