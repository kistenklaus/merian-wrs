...
struct PrefixPartitionBuffers {
    ...
    static Self allocate(const merian::ResourceAllocatorHandle& alloc,
        ...
        if (std::holds_alternative<DecoupledPrefixPartitionConfig>(config)) {
            ...
            using MethodBuffers = DecoupledPrefixPartition::Buffers;
            auto methodBuffers =
                MethodBuffers::allocate(alloc, N, methodConfig.partitionSize(), memoryMapping);
            assert(false);
...
template <prefix_partition_compatible T> class PrefixPartition {
  ...
  private:
    using Method = std::variant<DecoupledPrefixPartition, BlockWisePrefixPartition<T>>;
    static Method createMethod(const merian::ContextHandle& context,
                               const merian::ShaderCompilerHandle& shaderCompiler,
                               const PrefixPartitionConfig& config) {
        if (std::holds_alternative<DecoupledPrefixPartitionConfig>(config)) {
            return DecoupledPrefixPartition(context, shaderCompiler,
  ...
  public:
    ...
    void run(const merian::CommandBufferHandle& cmd,
        ...
        if (std::holds_alternative<DecoupledPrefixPartition>(m_method)) {
            assert(std::holds_alternative<Buffers::DecoupledInternals>(buffers.m_internalBuffers));
            DecoupledPrefixPartition::Buffers methodBuffers;
            assert(false); // TODO
