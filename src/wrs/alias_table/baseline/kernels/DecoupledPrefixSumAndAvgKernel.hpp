...
class DecoupledPrefixSumAndAverageKernel {
  ...
  public:
    DecoupledPrefixSumAndAverageKernel(const merian::ContextHandle& context,
        ...
        const merian::ShaderModuleHandle shader =
            context->shader_compiler->find_compile_glsl_to_shadermodule(
                context, "src/wrs/alias_table/baseline/kernels/decoupled_prefix_sum_and_avg.comp",
    ...
    void dispatch(vk::CommandBuffer cmd,
                  ...
                  merian::BufferHandle out_avgPrefixSum,
                  merian::BufferHandle partitionDescriptorBuffer) {
        m_pipeline->bind(cmd);
        m_pipeline->push_descriptor_set(cmd, in_elements, out_avgPrefixSum, partitionDescriptorBuffer);
